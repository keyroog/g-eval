...
def run_test_tc(input_file, single_template_path, output_file, offset_file, api_keys_file, max_requests_per_minute=10):
    """
    ...
    for record_idx, instance in enumerate(data[offset["record"]:], start=offset["record"]):
        try:
            ...
            for response_idx, response_entry in enumerate(responses[offset["response"]:], start=offset["response"]):
                try:
                    print(f"Processing record {record_idx + 1}, response {response_idx + 1}...")
