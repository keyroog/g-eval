...
def run_test(input_file, single_template_path, full_template_path, output_file, offset_file, max_requests_per_minute=10):
    ...
    for idx, instance in enumerate(data[offset:], start=offset):
        try:
            ...
            naturalness, coherence, engagingness, groundedness = None, None, None, None
            for evaluation in evaluations:
                lines = evaluation.split("\n")
                for line in lines:
                    if "Naturalness" in line and ":" in line:
                        try:
                            naturalness = int(line.split(":")[1].strip())
                        except ValueError:
                    ...
                    if "Engagingness" in line and ":" in line:
                        try:
                            engagingness = int(line.split(":")[1].strip())
                        except ValueError:
                            print(f"Errore nel parsing di Engagingness: {line}")
                    if "Groundedness" in line and ":" in line:
                        try:
                            groundedness = int(line.split(":")[1].strip())
                        except ValueError:
                            print(f"Errore nel parsing di Groundedness: {line}")
